...
function filterData(isPagination = false) {
    ...
    let idNhanVien = $('.tomselect-nhanVien').val();
    if (!isPagination) {
        firstLoad = true;
    }
    ...
    if (!idKhoHang || isNaN(idKhoHang) || parseInt(idKhoHang) <= 0) {
        toastr.error("Vui lòng chọn kho hàng hợp lệ");
        return false;
    }

    if (!idNhanVien || isNaN(idNhanVien) || parseInt(idNhanVien) <= 0) {
        toastr.error("Vui lòng chọn nhân viên hợp lệ");
        return false;
    }
    function parseDMY(s) {
        const p = s.split('-');
...
$('#btnExportExcel').off('click').on('click', function (e) {
    ...
    const idNhanVien = $('.tomselect-nhomHang').val();
...
function doExportPdf(finalData, btnElem) {
    const requestData = {
        ...
        idNhanVien: $('.tomselect-nhanVien').val(),
        doanhNghiep: window.doanhNghiep || null
    };
