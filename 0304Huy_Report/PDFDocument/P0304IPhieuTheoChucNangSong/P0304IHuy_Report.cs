    ...
    public class P0304IReportTemplatePDF : IDocument
    {
        ...
        public void Compose(IDocumentContainer container)
        {
            var sinhHoa = (_data.SinhHieus ?? Enumerable.Empty<SinhHieuModel>())
                .OrderBy(x => x.NgayKhaoSat ?? DateTime.MinValue) // bảo đảm cột thời gian render trái → phải
                .ToList();
            ...
            container.Page(page =>
            {
                    ...
                    col.Item().Padding(5).Row(row =>
                    {
                        ...
                        row.RelativeItem().AlignCenter()
                            .Text(text =>
                            {
                                ...
                                text.Span($"{_data?.ThongTinBenhNhan?.Tuoi ?? 0}").FontSize(10);
                            });
                ...
                var listNgay = _data.SinhHieus?.Select(sh => sh.NgayKhaoSat?.ToString("dd-MM-yyyy")).ToList();
                ...
                {
                    // Xác định giá trị đường kẻ trên của ô
                    ...
                    // Quy đổi sang dải 0..10
                    double pTop = offsetRatio * 10.0;
                    double pBot = 10.0 - pTop;
                ...
                page.Content().PaddingVertical(6).Element(e =>
                {
                    e.Table(table =>
                    {
                        ...
                        {
                            table.Cell().Element(CellStyle).AlignCenter().Text($"{ngay}");
                        }
                        ...
                        table.Cell().Element(CellStyle).AlignCenter().Text("Mạch (1/ph)");
                        table.Cell().Element(CellStyle).AlignCenter().Text("Nhiệt độ (C)");
                        table.Cell().Element(CellStyle).AlignCenter().Text("");
                        for (int i = 0; i < listMach.Count && i < listNhietDo.Count; i++)
                        {
                            ...
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("160");
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("41");
                            if ((mach > 140 && mach <= 160) || (nhietDo > 40 && nhietDo <= 41))
                            {
                                if ((mach > 140 && mach <= 160) && !(nhietDo > 40 && nhietDo <= 41))
                                    table.Cell().Element(c => c.CellStyleCham(paddingTop: ptMach, paddingBottom: pbMach)).AlignCenter().Text($"{mach}").FontColor("#FF0000");
                                else if (!(mach > 140 && mach <= 160) && (nhietDo > 40 && nhietDo <= 41))
                                    table.Cell().Element(c => c.CellStyleCham(paddingTop: ptNhiet, paddingBottom: pbNhiet)).AlignCenter().Text($"{nhietDo}").FontColor("#0000FF");
                                else if ((mach > 140 && mach <= 160) && (nhietDo > 40 && nhietDo <= 41))
                                {
                                    table.Cell().AlignCenter().Row(row =>
                                    {
                                        row.AutoItem()
                            ...
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("140");
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("40");
                            ...
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("120");
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("39");
                            ...
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("100");
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("38");
                                ...
                                else if ((mach > 80 && mach <= 100) && (nhietDo > 37 && nhietDo <= 38))
                                {
                                    table.Cell().AlignCenter().Border(1).Row(row =>
                                    {
                                        row.RelativeItem(1)
                                           ...
                                           .Text(text =>
                                           {
                                               text.Span($"{mach} ").FontColor("#FF0000");
                                               text.Span("●").FontColor("#FF0000");
                                               text.Span($"{nhietDo}").FontColor("#0000FF");
                            ...
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("80");
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("37");
                            ...
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("60");
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("36");
                            ...
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("40");
                            table.Cell().Element(CellStyleTop).AlignCenter().Text("35");
                        ...
                        table.Cell().ColumnSpan(2).Element(CellStyle).AlignLeft().Text("1. Huyết áp");
                        foreach(var huyetAp in listHuyetAp)
                        {
                            table.Cell().Element(CellStyle).AlignCenter().Text($"{huyetAp}");
                        }
                        table.Cell().ColumnSpan(2).Element(CellStyle).AlignLeft().Text("2. Cân nặng");
                        foreach (var canNang in listCanNang)
                        {
                            table.Cell().Element(CellStyle).AlignCenter().Text($"{canNang}");
                        }
                        table.Cell().ColumnSpan(2).Element(CellStyle).AlignLeft().Text("3. Nhịp thở");
