...
function filterData(isPagination = false) {
    ...
    $.ajax({
        url: '/bang_ke_bien_lai_tam_ung/filter',
        type: 'POST',
        data: payload,
...
function doExportExcel(finalData, btn, originalHtml) {
    ...
    $.ajax({
        url: '/bang_ke_bien_lai_tam_ung/export/excel',
        type: 'POST',
        ...
        success: function (data, status, xhr) {
            ...
            a.href = url;
            a.download = `BangKeBienLaiTamUng_${requestData.fromDate || 'all'}_den_${requestData.toDate || 'now'}.xlsx`;
...
function doExportPdf(finalData, btnElem) {
    ...
    fetch("/bang_ke_bien_lai_tam_ung/export/pdf", {
        method: "POST",
        ...
        .then(blob => {
            ...
            const a = document.createElement("a");
            a.href = url;
            a.download = `BangKeBienLaiTamUng_${requestData.fromDate || 'all'}_den_${requestData.toDate || 'now'}.pdf`;
            a.click();
            window.URL.revokeObjectURL(url);
        ...
        data.forEach((item, index) => {
            ...
            const row = `
                <tr>
                    ...
                    <td class="text-nowrap text-center">${item.soBL || item.SoBL || 'Không rõ'}</td>
                    <td class="text-nowrap text-center">${item.soQuyen || item.SoQuyen || 'Không rõ'}</td>
                    <td class="text-nowrap text-end">${formatCurrency(item.thuPhi || item.ThuPhi || 0)}</td>
