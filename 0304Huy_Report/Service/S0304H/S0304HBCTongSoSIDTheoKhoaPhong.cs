...
using P0304H.PDFDocument;

namespace S0304HBCTongSoSIDTheoKhoaPhong.Services
{
    public class S0304HBCTongSoSIDTheoKhoaPhongService : I0304HBCTongSoSIDTheoKhoaPhongService
    {
        private readonly M0304Context _context;
        private readonly ILogger<S0304HBCTongSoSIDTheoKhoaPhongService> _logger;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly I0304ThongTinDoanhNghiep _thongTinDoanhNghiepService;
        private readonly IWebHostEnvironment _env;

        public S0304HBCTongSoSIDTheoKhoaPhongService(M0304Context context, ILogger<S0304HBCTongSoSIDTheoKhoaPhongService> logger, IHttpContextAccessor httpContextAccessor,
        ...
        public async Task<M0304HBCTongSoSIDTheoKhoaPhongResponse> GetBCTongSoSIDTheoKhoaPhong(string ngayBatDau, string ngayKetThuc, long idCN, int page = 1, int pageSize = 20)
        {
            ...
            else
            {
                ...
                return new M0304HBCTongSoSIDTheoKhoaPhongResponse
                {
                    BCTongSoSIDTheoKhoaPhong = new M0304HPagedResult<M0304HBCTongSoSIDTheoKhoaPhong>
                    {
                        Success = false,
            ...
            var allData = await _context.M0304HBCTongSoSIDTheoKhoaPhongs
                .FromSqlRaw("EXEC dbo.S0304_BCTongSoSIDTheoKhoaPhong @TuNgay, @DenNgay, @IDCN",
        ...
        public async Task<byte[]> ExportBCTongSoSIDTheoKhoaPhongPdfAsync(ExportRequest request, ISession session)
        {
            ...
            var data = request.Data ?? new List<M0304HBCTongSoSIDTheoKhoaPhong>();
            var document = new P0304HReportTemplatePDF(data, request.FromDate, request.ToDate, doanhNghiepObj, logoPath);

            var pdfBytes = document.GeneratePdf();
            return pdfBytes;
        }
        public async Task<byte[]> ExportBCTongSoSIDTheoKhoaPhongExcelAsync(ExportRequest request, ISession session)
        {
            var doanhNghiepObj = GetDoanhNghiepFromRequestOrSession(request, session);
            var logoPath = Path.Combine(_env.WebRootPath, "dist", "img", "logo.png");

            var data = request.Data ?? new List<M0304HBCTongSoSIDTheoKhoaPhong>();
            var document = new P0304HExcelReportTemplate(data, request.FromDate, request.ToDate, doanhNghiepObj, logoPath);
