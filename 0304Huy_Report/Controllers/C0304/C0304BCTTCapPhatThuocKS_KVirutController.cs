...
namespace C0304BCTTCapPhatThuocKS_KVirut.Controllers
{
    [Route("bao_cao_thong_tin_cap_phat_thuoc_khang_sinh_khang_virut")]
    public class C0304BCTTCapPhatThuocKS_KVirutController : Controller
    {
        //private string _maChucNang = "/bao_cao_thong_tin_cap_phat_thuoc_khang_sinh_khang_virut";
        //private IMemoryCachingServices _memoryCache;
        private readonly ILogger<C0304BCTTCapPhatThuocKS_KVirutController> _logger;
        private readonly M0304Context _context;
        private readonly I0304ThongTinDoanhNghiep _thongTinDoanhNghiepService;
        private readonly IWebHostEnvironment _env;

        public C0304BCTTCapPhatThuocKS_KVirutController(ILogger<C0304BCTTCapPhatThuocKS_KVirutController> logger,
        ...
        public async Task<IActionResult> Index()
        {
            ...
            return View("~/Views/V0304/V0304BCTTCapPhatThuocKS_KVirut/Index.cshtml");
        }
            ...
            long? idHangHoa = null, int page = 1, int pageSize = 20)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Service trả về lỗi: {Message}", result.BCTTCapPhatThuocKS_KVirut.Message);
                    return Json(new { success = false, message = result.BCTTCapPhatThuocKS_KVirut.Message });
                }

                return Json(new
                {
                    success = true,
                    message = result.BCTTCapPhatThuocKS_KVirut.Message,
                    data = result.BCTTCapPhatThuocKS_KVirut.Data,
                    totalRecords = result.BCTTCapPhatThuocKS_KVirut.TotalRecords,
                    totalPages = result.BCTTCapPhatThuocKS_KVirut.TotalPages,
                    currentPage = result.BCTTCapPhatThuocKS_KVirut.CurrentPage,
        ...
        public async Task<IActionResult> ExportToExcel([FromBody] ExportRequest request)
        {
            ...
            string fileName = $"BCTTCapPhatThuocKS_KVirutNgoaiTru_{request.FromDate ?? "all"}_den_{request.ToDate ?? "now"}.xlsx";
            ...
            long? idKhoHang, long? idNhomHang = null, long? idHangHoa = null, int page = 1, int pageSize = 20)
        {
            ...
            else
            {
                ...
                return new M0304BCTTCapPhatThuocKS_KVirutResponse
                {
                    BCTTCapPhatThuocKS_KVirut = new M0304BCTTCapPhatThuocKS_KVirutPagedResult<M0304BCTTCapPhatThuocKS_KVirut>
                    {
                        Success = false,
        ...
        private async Task<byte[]> ExportBaoCaoGoiKhamExcelAsync(ExportRequest request, ISession session)
        {
            ...
            var data = request.Data ?? new List<M0304BCTTCapPhatThuocKS_KVirut>();
            var document = new P0304BCTTCapPhatThuocKS_KVirutExcelReportTemplate(data, request.FromDate, request.ToDate, doanhNghiepObj, logoPath);
