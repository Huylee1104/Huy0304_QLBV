...
function filterData(isPagination = false) {
    ...
    $.ajax({
        url: '/bao_cao_thong_tin_cap_phat_thuoc_khang_sinh_khang_virut/filter',
        type: 'POST',
        data: payload,
...
function doExportExcel(finalData, btn, originalHtml) {
    ...
    $.ajax({
        url: '/bao_cao_thong_tin_cap_phat_thuoc_khang_sinh_khang_virut/export/excel',
        type: 'POST',
        ...
        success: function (data, status, xhr) {
            ...
            a.href = url;
            a.download = `BCTTCapPhatThuocKS_KVirut_${requestData.fromDate || 'all'}_den_${requestData.toDate || 'now'}.xlsx`;
...
function doExportPdf(finalData, btnElem) {
    ...
    fetch("/bao_cao_thong_tin_cap_phat_thuoc_khang_sinh_khang_virut/export/pdf", {
        method: "POST",
...
function updateTable(response) {
    ...
    if (data.length > 0) {
        data.forEach((item, index) => {
            ...
            const row = `
                <tr>
                    ...
                    <td class = "text-start text-nowrap">${item.lieuDung || item.LieuDung || ''}</td>
        ...
        tbody.append('<tr><td colspan="19" class="text-center">Không có dữ liệu</td></tr>');
    }
}
...
$.getJSON("dist/data/json/Dm_NhomHang.json", dataNhomHang => {
    ...
    console.log("listNhomHang", listNhomHang);
...
function loadHangHoa() {
    $.getJSON("dist/data/json/HH_DM_HangHoa.json", dataHangHoa => {
        ...
        console.log("ID Nhóm hàng là", idNhomHang)
