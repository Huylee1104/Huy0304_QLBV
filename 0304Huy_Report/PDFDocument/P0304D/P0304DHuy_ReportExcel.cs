...
public class P0304DExcelReportTemplate
{
    ...
    public P0304DExcelReportTemplate(
    ...
    public byte[] GenerateExcel()
    {
        using (var wb = new XLWorkbook())
        {
            ...
            ws.Cell(currentRow, 2).Value = "BẢNG KÊ TẠM ỨNG";
            ...
            "Họ và tên", "Sổ quyển", "Số biên lai", "Thu phí", "Hủy", "Hoàn trả", "HTTT"
            };
            ...
            foreach (var nv in _danhSachNhanVien)
            {
                ...
                foreach (var item in dataNV)
                {
                    ...
                    ws.Cell(currentRow, 3).Value = item.NgayThu?.ToString("dd-MM-yyyy HH:mm:ss") ?? ""; AlignCellCenter(ws.Cell(currentRow, 3));
                    ...
                    ws.Cell(currentRow, 8).Value = item.SoBL ?? ""; AlignCellCenter(ws.Cell(currentRow, 8));
                    ws.Cell(currentRow, 9).Value = item.SoQuyen ?? ""; AlignCellCenter(ws.Cell(currentRow, 9));
                    ws.Cell(currentRow, 10).Value = item.ThuPhi ?? (decimal?)null;
            ...
            ws.Range(currentRow, 2, currentRow, 9).Merge();
            ws.Cell(currentRow, 2).Value = $"Số tiền phải nộp: {tongChenhLech:N0}";
            ws.Cell(currentRow, 2).Style.Font.Bold = true;
            currentRow++;
